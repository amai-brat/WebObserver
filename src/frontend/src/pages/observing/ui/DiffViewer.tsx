...
export const DiffViewer: React.FC<DiffViewerProps> = ({ observingId, entryId }) => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await observingApi.getEntryDiffPayload(observingId, entryId, ac.signal);
  ...
  return (
    <div className="m-8 flex flex-col w-full gap-2">
      <div className="text-white text-3xl text-center">Изменения:</div>
